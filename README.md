...
- 买入只使用 `LIMIT_MAKER`；`BID_ASK_MAKER` 可配置买一至买五，另外三种策略固定买一。配置金额作为基准，每张 BUY 在 `[基准×80%, 基准×120%]` USDT 内取随机整数（同时满足交易所最小额且不超过 `30 USDT`）；买单超时在配置基准的 `1.0～1.5` 倍内按单随机，超时后若订单仍在所选档位则继续挂，否则撤单，不转 IOC。
- 四种策略的每张买单都要求其挂单价严格低于分时图 1 分钟成交价的 MA(7) 和 MA(25)。系统预热最近 25 根分钟线并实时更新；分钟数据不足或过期时不挂买单，已挂买单不再满足条件时撤单对账。
...
`BREAK_EVEN_MAKER` 在买一挂买单，除所有策略共用的 MA(7)/MA(25) 门槛外，还要求买价相对 60 秒滚动中间价的折让覆盖预估往返 Maker 费率和 5 bps 缓冲；也可填写手动参考价（均线条件仍须满足）。初始卖价和超时重挂价均守实际持仓净保本价，不追求或限制每日成交量，只持续记录成交统计。
