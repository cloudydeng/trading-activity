...
- 买入只使用 `LIMIT_MAKER`；有效同币种并发上限大于 1 时，四种策略的新 BUY 统一挂实时买四；上限为 1 时，`BID_ASK_MAKER` 使用其配置的买一至买五，另外三种策略使用买一。配置金额作为基准，每张 BUY 在 `[基准×80%, 基准×120%]` USDT 内取随机整数（同时满足交易所最小额且不超过 `30 USDT`）；买单超时在配置基准的 `1.0～1.5` 倍内按单随机，超时后若订单仍在实际生效档位则继续挂，否则撤单，不转 IOC。
...
`BID_ASK_MAKER` 在同币种并发上限不大于 1 时按 `bidAskEntryBookLevel` 选择买一至买五挂买单（默认买一），上限大于 1 时与其他策略一样改用买四；初始卖价取卖一和“买入均价 + `bidAskInitialSellMarkupTicks` 个 tick”的较高值，加价默认 `1 tick`；
`BUY_PRICE_MAKER` 在并发上限为 1 时挂买一，初始卖价直接取实际买入均价并按 tick 向上取整；
`FEE_AWARE_MAKER` 在并发上限为 1 时同样挂买一，但增加买入锚点，并让初始卖价不低于手续费保护价。
`BREAK_EVEN_MAKER` 在并发上限为 1 时挂买一；除所有策略共用的 MA(7) 门槛外，还要求买价相对 60 秒滚动中间价的折让覆盖预估往返 Maker 费率和 5 bps 缓冲；也可填写手动参考价（均线条件仍须满足）。初始卖价和超时重挂价均守实际持仓净保本价，不追求或限制每日成交量，只持续记录成交统计。
