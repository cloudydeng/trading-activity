...
产品、运营和测试请先阅读 [Maker 策略产品说明](docs/strategy-behavior.md)。

## 安全边界

- BUY 部分成交会立即进入 SELL 管理，成交以账户事件流和 REST 对账为准。
- 买入只使用 `LIMIT_MAKER`；同一币种跨所有 API Key 同时最多只有一张由本系统管理的 BUY（含报单中的占位）。有效同币种并发上限大于 1 且已有活动 SELL 时，四种策略的新 BUY 统一挂实时买四；没有活动 SELL 时，`BID_ASK_MAKER` 使用其配置的买一至买五，另外三种策略使用买一。配置金额作为基准，每张 BUY 在 `[基准×80%, 基准×120%]` USDT 内取随机整数（同时满足交易所最小额且不超过 `30 USDT`）；买单超时在配置基准的 `1.0～1.5` 倍内按单随机，超时后若订单仍在实际生效档位则继续挂，否则撤单，不转 IOC。
- 四种策略的每张买单都要求其挂单价严格低于分时图 1 分钟成交价的 MA(7)，MA(25) 不再阻挡买入。系统预热最近 7 根分钟线并实时更新；分钟数据不足或过期时不挂买单，已挂买单不再满足条件时撤单对账。MA(7) 合格后，候选买价还须与同币种跨 API Key 最近一次实际 BUY 成交价相差不超过 0.5%（以该成交价为分母，涨跌均计算）；超过时只暂停新 BUY，从首次触发起最多等待 1 小时，价差提前回到范围内则清零计时，超时后仅放开这项价差限制。等待时间跨重启保留，已有 SELL 照常管理；没有历史 BUY 成交时不施加这项价差门槛。
- BUY 一旦真实成交（包括达到最小可卖额的部分成交），立即撤销剩余买单并对账，再按当前策略挂第一张卖单：`BID_ASK_MAKER` 使用卖一/买入价上方可配置 tick 底线，`BUY_PRICE_MAKER` 使用实际买入均价，`FEE_AWARE_MAKER` 使用手续费保护价，`BREAK_EVEN_MAKER` 根据实际持仓成本及卖出费率保护净保本价。
- 每张卖单按配置基准的 `1.0～1.5` 倍随机时间检查（`BUY_PRICE_MAKER` 和 `BREAK_EVEN_MAKER` 默认基准 1800 秒，其他策略默认 120 秒）；普通策略按最新卖一重挂，`BREAK_EVEN_MAKER` 重挂价格取最新卖一和净保本价中的较高值，绝不主动降到净保本线以下。
- 自动交易流程没有价格止损、止损冷却或 MARKET 卖出；MARKET 只保留给人工授权清仓。库存对账与防重复卖出始终是强制保护。
- 每个 API 账户拥有一个账户会话和一条 User Data WebSocket，可配置最多 5 个交易对同时运行。每个交易对拥有独立策略状态机、行情流、订单、持仓恢复和统计；账户余额、BNB 检查、买单提交、总持仓风险与总回撤保护由同一账户共享协调。同一个交易对跨 API key 默认按完整买卖轮次串行；并发 N 表示最多 N 个账户同时占用完整交易轮次，但同币种同一时刻只允许一个买入轮次处于 BUYING。每个名额从挂 BUY 开始，经过持仓和 SELL，直到全部卖完才释放。等待账户进入严格的按币种 FIFO 队列，重复重试不会插队。不同 API key 的同币种新 BUY 报单相对上一账户的买单、卖单或完整轮次结束再额外错开至少 2 秒，已有仓位的 SELL 不延迟；并发设置保存到 SQLite 并立即生效。
...
`BID_ASK_MAKER` 在没有同币种活动 SELL（或并发上限不大于 1）时按 `bidAskEntryBookLevel` 选择买一至买五挂买单（默认买一），仅在并发上限大于 1 且有活动 SELL 时与其他策略一样改用买四；初始卖价取卖一和“买入均价 + `bidAskInitialSellMarkupTicks` 个 tick”的较高值，加价默认 `1 tick`；
`BUY_PRICE_MAKER` 在没有触发买四覆盖时挂买一，初始卖价直接取实际买入均价并按 tick 向上取整；
`FEE_AWARE_MAKER` 在没有触发买四覆盖时同样挂买一，但增加买入锚点，并让初始卖价不低于手续费保护价。
`BREAK_EVEN_MAKER` 在没有触发买四覆盖时挂买一；除所有策略共用的 MA(7) 门槛外，还要求买价相对 60 秒滚动中间价的折让覆盖预估往返 Maker 费率和 5 bps 缓冲；也可填写手动参考价（均线条件仍须满足）。初始卖价和超时重挂价均守实际持仓净保本价，不追求或限制每日成交量，只持续记录成交统计。
保本策略有两个独立时间字段，默认都为 `1800 秒`：卖单复查间隔基准按每张单随机取 `1～1.5` 倍；最长持仓等待从首笔买入成交开始按固定期限计算。后者到期后停止后续买入并提示人工关注，已有卖单继续守净保本价，不自动亏损卖出；最长持仓等待填 `0` 可关闭期限。
其中 `1 bps = 0.01%`；单轮最大追高上限为 `100 bps`（1%），累计最大追高上限为 `1000 bps`（10%）。
卖单到达检查时间时，如果仍在卖一就保留并重新计时；如果不在卖一，则撤单对账并直接按最新卖一重挂。
除 `BREAK_EVEN_MAKER` 外，超时阶段优先释放仓位，不再强制买入价或手续费保护价底线。通过控制台给账户新增交易对时，若该交易对没有已有的持久化或账户级策略配置，系统会保存一份初始配置：`BUY_PRICE_MAKER`、单笔金额基准 `12 USDT`、每日成交量上限 `10000 USDT`、卖单超时基准 `119 秒`；已有交易对及已有配置不会被覆盖。卖单的实际检查窗口仍按基准随机取 `1～1.5` 倍。
