...
- 买入只使用 `LIMIT_MAKER`；`BID_ASK_MAKER` 可配置买一至买五，另外两种策略固定买一。配置金额作为基准，每张 BUY 在 `[基准-8, 基准+2]` USDT 内取随机整数（同时满足交易所最小额且不超过 `30 USDT`）；买单超时在配置基准的 `1.0～1.5` 倍内按单随机，超时后若订单仍在所选档位则继续挂，否则撤单，不转 IOC。
