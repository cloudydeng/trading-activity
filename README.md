...
- 每张卖单按配置时间检查（默认 2 分钟）；超时先比较当前订单和最新卖一价，若仍在卖一则保留原 `LIMIT GTC SELL` 并重新计时，只有不在卖一时才撤单、完成成交与库存对账，再将剩余数量按最新卖一价重挂，之后按配置时间重复，直至空仓。
...
此阶段优先快速成交和释放仓位，不再强制手续费保护价。未配置的交易对回退到 `CURRENT`。
