...
- 买入只使用 `LIMIT_MAKER`；`BID_ASK_MAKER` 可配置买一至买五，另外两种策略固定买一。配置金额作为基准，每张 BUY 在 `[基准×80%, 基准×120%]` USDT 内取随机整数（同时满足交易所最小额且不超过 `30 USDT`）；买单超时在配置基准的 `1.0～1.5` 倍内按单随机，超时后若订单仍在所选档位则继续挂，否则撤单，不转 IOC。
...
- 每张卖单按配置基准的 `1.0～1.5` 倍随机时间检查（`BUY_PRICE_MAKER` 默认基准 1800 秒，其他策略默认 120 秒）；若原单仍在卖一，则保留原单并重新抽取下一次检查时间；若不在卖一，则撤单、对账并直接按最新卖一重挂，新单也重新随机超时。超时重挂不再使用买入价或手续费保护价底线。
...
超时阶段优先释放仓位，不再强制买入价或手续费保护价底线。通过控制台给账户新增交易对时，若该交易对没有已有的持久化或账户级策略配置，系统会保存一份初始配置：`BUY_PRICE_MAKER`、单笔金额基准 `12 USDT`、每日成交量上限 `10000 USDT`、卖单超时基准 `119 秒`；已有交易对及已有配置不会被覆盖。卖单的实际检查窗口仍按基准随机取 `1～1.5` 倍。
