...
- BUY 一旦真实成交（包括达到最小可卖额的部分成交），立即撤销剩余买单并按实际加权买入均价（按 tick 向上取整）挂 `LIMIT GTC SELL`；例如买入均价 `0.862` 就挂卖价 `0.862`，不再抬到手续费目标价，库存对账与防重复卖出仍为强制保护。
- ENSO 入场允许轻度卖一占优（买一力量差最低 `-0.35`）；五档深度方向仅展示、不再阻止入场，深度过期、主动卖压、快速下跌和行情过期仍会阻止入场或提前撤单。
- 主动成交方向在 3 秒窗口内至少累计 3 笔后才参与判断；样本不足按中性处理，避免单笔小卖单误触发撤单。
